package shared.comLuz.infraestructure.hibernate;
...
public final class HibernateConfigurationFactory {
    ...
    public DataSource dataSource(
        ...
        Resource mysqlResource = resourceResolver.getResource(String.format(
            "classpath:resources/database/%s.sql",
            databaseName
        ));
    ...
    private List<String> subdirectoriesFor(String contextName) {
        String path = "./src/" + contextName + "/main/comLuz/" + contextName + "/";

        String[] files = new File(path).list((current, name) -> new File(current, name).isDirectory());

        if (null == files) {
            path  = "./main/comLuz/" + contextName + "/";
